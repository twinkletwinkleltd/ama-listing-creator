 ...
 * portal_user cookie) on every request to /apps/listing/*.
 *
 * Requirements:
 * - All routes (pages + API) require X-Portal-User
 * - Missing/empty header:
 *     - API routes (/apps/listing/api/*) → 401 JSON
...
const PORTAL_LOGIN_URL = '/login';   // portal-relative (outside basePath)
...
export function middleware(req: NextRequest) {
  ...
  res.headers.set('x-ama-auth-checked', 'true');
  return res;
}
